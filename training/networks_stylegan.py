...
def _blur3d(x, f=[1,2,1], normalize=True, flip=False, stride=1):
    ...
    x = tf.nn.conv3d(x, f, strides=strides, padding='SAME', data_format='NCDHW')
...
def blur3d(x, f=[1,2,1], normalize=True):
    with tf.variable_scope('Blur3D'):
        @tf.custom_gradient
        def func(x):
...
def upscale3d(x, factor=2):
    with tf.variable_scope('Upscale3D'):
        @tf.custom_gradient
        def func(x):
            y = _upscale3d(x, factor)
            @tf.custom_gradient
            def grad(dy):
                dx = _downscale3d(dy, factor, gain=factor**2)
                return dx, lambda ddx: _upscale3d(ddx, factor)
            return y, grad
        return func(x)
...
def downscale3d(x, factor=2):
    with tf.variable_scope('Downscale3D'):
        @tf.custom_gradient
        def func(x):
            y = _downscale3d(x, factor)
            @tf.custom_gradient
            def grad(dy):
                dx = _upscale3d(dy, factor, gain=1/factor**2)
                return dx, lambda ddx: _downscale3d(ddx, factor)
            return y, grad
        # print('downscale ', temp.shape)
        return func(x)
...
def conv3d(x, fmaps, kernel, **kwargs):
...
def upscale3d_conv3d(x, fmaps, kernel, fused_scale='auto', **kwargs):
    assert kernel >= 1 and kernel % 2 == 1
    ...
    if not fused_scale:
        return conv3d(upscale3d(x), fmaps, kernel, **kwargs)

    # Fused => perform both ops simultaneously using tf.nn.conv2d_transpose().
    w = get_weight([kernel, kernel, kernel, x.shape[1].value, fmaps], **kwargs)
    w = tf.transpose(w, [0, 1, 2, 4, 3]) # [kernel, kernel, fmaps_out, fmaps_in]
...
def conv3d_downscale3d(x, fmaps, kernel, fused_scale='auto', **kwargs):
    assert kernel >= 1 and kernel % 2 == 1
    assert fused_scale in [True, False, 'auto']
    if fused_scale == 'auto':
    ...
    # Not fused => call the individual ops directly.
    if not fused_scale:
        return downscale3d(conv3d(x, fmaps, kernel, **kwargs))
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    with tf.variable_scope('MinibatchStddev'):
        ...
        y = tf.reshape(x, [group_size, -1, num_new_features, s[1]//num_new_features, s[2], s[3], s[4]])   # [GMncHW] Split minibatch into M groups of size G. Split channels into n channel groups c.
        y = tf.cast(y, tf.float32)                              # [GMncHW] Cast to FP32.
        y -= tf.reduce_mean(y, axis=0, keepdims=True)           # [GMncHW] Subtract mean over group.
        y = tf.reduce_mean(tf.square(y), axis=0)                # [MncHW]  Calc variance over group.
        y = tf.sqrt(y + 1e-8)                                   # [MncHW]  Calc stddev over group.
        y = tf.reduce_mean(y, axis=[2,3,4,5], keepdims=True)      # [Mn111]  Take average over fmaps and pixels.
        y = tf.reduce_mean(y, axis=[2])                         # [Mn11] Split channels into c channel groups
        y = tf.cast(y, x.dtype)                                 # [Mn11]  Cast back to original data type.
        y = tf.tile(y, [group_size, 1, s[2], s[3], s[4]])             # [NnHW]  Replicate over group and pixels.
...
def G_mapping(
    latents_in,                             # First input: Latent vectors (Z) [minibatch, latent_size].
    labels_in,                              # Second input: Conditioning labels [minibatch, label_size].
    latent_size             = 2048,          # Latent vector (Z) dimensionality.
    label_size              = 0,            # Label dimensionality, 0 if no labels.
    dlatent_size            = 2048,          # Disentangled latent (W) dimensionality.
    dlatent_broadcast       = None,         # Output disentangled latent (W) as [minibatch, dlatent_size] or [minibatch, dlatent_broadcast, dlatent_size].
    mapping_layers          = 4,            # Number of mapping layers.
    mapping_fmaps           = 2048,          # Number of activations in the mapping layers.
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    # Inputs.
    if label_size:
        latent_size = latent_size-label_size
    ...
    if label_size:
        with tf.variable_scope('LabelConcat'):
            ...
            x = tf.concat([x, labels_in], axis=1)
...
def G_synthesis(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def blur(x): return blur3d(x, blur_filter) if blur_filter else x
    ...
    with tf.variable_scope('4x4'):
        if const_input_layer:
            with tf.variable_scope('Const'):
                x = tf.get_variable('const', shape=[1, nf(1), 4, 4, 4], initializer=tf.initializers.ones())
                x = layer_epilogue(tf.tile(tf.cast(x, dtype), [tf.shape(dlatents_in)[0], 1, 1, 1, 1]), 0)
        else:
            with tf.variable_scope('Dense'):
                x = dense(dlatents_in[:, 0], fmaps=nf(1)*64, gain=gain/4, use_wscale=use_wscale) # tweak gain to match the official implementation of Progressing GAN
                x = layer_epilogue(tf.reshape(x, [-1, nf(1), 4, 4, 4]), 0)
        with tf.variable_scope('Conv'):
            x = layer_epilogue(conv3d(x, fmaps=nf(1), kernel=3, gain=gain, use_wscale=use_wscale), 1)

    # Building blocks for remaining layers.
    def block(res, x): # res = 3..resolution_log2
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            with tf.variable_scope('Conv0_up'):
                x = layer_epilogue(blur(upscale3d_conv3d(x, fmaps=nf(res-1), kernel=3, gain=gain, use_wscale=use_wscale, fused_scale=fused_scale)), res*2-4)
                tf.add_to_collection('checkpoints',x)
            with tf.variable_scope('Conv1'):
                x = layer_epilogue(conv3d(x, fmaps=nf(res-1), kernel=3, gain=gain, use_wscale=use_wscale), res*2-3)
            return x
    ...
    if structure == 'recursive':
        ...
        def grow(x, res, lod):
            y = block(res, x)
            img = lambda: upscale3d(torgb(res, y), 2**lod)
            img = cset(img, (lod_in > lod), lambda: upscale3d(tflib.lerp(torgb(res, y), upscale3d(torgb(res - 1, x)), lod_in - lod), 2**lod))
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def fromrgb(x, res): # res = 2..resolution_log2
        with tf.variable_scope('FromRGB_lod%d' % (resolution_log2 - res)):
            return act(apply_bias(conv3d(x, fmaps=nf(res-1), kernel=1, gain=gain, use_wscale=use_wscale)))
    def block(x, res): # res = 2..resolution_log2
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res >= 3: # 8x8 and up
                with tf.variable_scope('Conv0'):
                    # print('res one',x.shape)

                    x = act(apply_bias(conv3d(x, fmaps=nf(res-1), kernel=3, gain=gain, use_wscale=use_wscale)))
                    tf.add_to_collection('checkpoints',x)

                with tf.variable_scope('Conv1_down'):
                    # print('res',res)
                    x = act(apply_bias(conv3d_downscale3d(blur(x), fmaps=nf(res-2), kernel=3, gain=gain, use_wscale=use_wscale, fused_scale=fused_scale)))
                    # print('res don',x.shape)
                ...
                with tf.variable_scope('Dense0'):
                    x = act(apply_bias(dense(x, fmaps=128, gain=gain, use_wscale=use_wscale)))
    ...
    if structure == 'recursive':
        ...
        def grow(res, lod):
            ...
            if res > 2: y = cset(y, (lod_in > lod), lambda: tflib.lerp(x, fromrgb(downscale3d(images_in, 2**(lod+1)), res - 1), lod_in - lod))
            return y()
