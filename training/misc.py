...
def save_mri_image(image, filename, drange=[0,1]):

    image = image[0][0]
    image = adjust_dynamic_range(image, drange, [0,255])
    image = np.rint(image).clip(0, 255).astype(np.uint8)
    mri = nib.Nifti1Image(image, np.eye(4))
...
def setup_snapshot_image_grid(G, training_set,
    ...
    if size == '4k':
        gw = np.clip(3840 // G.output_shape[3], 7, 7)
        gh = np.clip(2160 // G.output_shape[2], 6, 6)
...
def setup_test_snapshot_image_grid(G, training_set,
    size    = '1080p',      # '1080p' = to be viewed on 1080p display, '4k' = to be viewed on 4k display.
    layout  = 'random'):    # 'random' = grid contents are selected randomly, 'row_per_class' = each row corresponds to one class label.
    ...
    # if size=='custom':
    #     gw = np.clip(1920 // G.output_shape[3], 3, 32)
    #     gh = np.clip(1536 // G.output_shape[2], 2, 32)

    # Initialize data arrays.
    # np.random.seed(23)
    reals = np.zeros([gw * gh] + training_set.shape, dtype=training_set.dtype)
    labels = np.zeros([gw * gh, training_set.label_size], dtype=training_set.label_dtype)
    latents = np.repeat([np.zeros((7, *G.input_shape[1:]))], (gw*gh)//7, axis=0).reshape((gw*gh,*G.input_shape[1:]))
    ...
    # temp = latents[1]
    # temp = np.random.randn(1, *G.input_shape[1:])
    # for i in range(1,5):
    #     latents[i] = (latents[0]*(i)+temp*(5-i))/4
    # latents[5]=latents[4]
    # latents[4]=latents[3]
    # latents[3]=(latents[0]+temp)/2.0
