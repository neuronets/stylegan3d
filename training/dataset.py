...
def parse_tfrecord_tf(record):
    features = tf.parse_single_example(record, features={
        # 'axis': tf.FixedLenFeature([1], tf.int64),
...
class SyntheticDataset:
    def __init__(self, resolution=1024, num_channels=3, dtype='uint8', dynamic_range=[0,255], label_size=0, label_dtype='float32'):
        ...
        self.shape              = [num_channels, resolution, resolution, resolution]
        self.dtype              = dtype
    ...
    def get_minibatch_tf(self): # => images, labels
        with tf.name_scope('SyntheticDataset'):
            ...
            shape = [self.shape[0], self.shape[1] // shrink, self.shape[2] // shrink, self.shape[3] // shrink]
